...
// Default backend API URL - can be configured via environment variable or changed to team's backend
// Use team's Express.js backend (port 4000) or original FastAPI backend (port 5000)
const API_BASE_URL = window.APP_CONFIG?.BACKEND_URL || window.API_BASE_URL || 'http://127.0.0.1:5000';

// Supabase client for authentication (loaded from HTML, available globally)
...
const app = {
    ...
    init() {
        ...
        this.initTheme();
        this.initMermaid();
        // Always start on the auth view and then attempt to restore the session
        this.showView('auth');
        if (this.state.isAuthenticated) {
            // Attempt to validate the stored token before showing the dashboard
            ...
            this.getCurrentUser()
                ...
                .catch(() => {
                    // If token is invalid or backend unavailable, clear saved auth
    ...
    // Get current user from team's backend /api/me endpoint
    ...
    async markQuestion(userAnswer, maxScore, userConfidence) {
        ...
        throw new Error('Unable to mark question');
    },
    ...
    // Handle Supabase authentication for login/signup
    async handleSupabaseAuth(action, email, password) {
        try {
    ...
    // Handle login or signup form submission using Supabase auth
    async handleAuth(event) {
        ...
        try {
            const success = await this.handleSupabaseAuth(
            ...
            if (success) {
                ...
                this.showError('Authentication failed. Please check your email and password.');
            }
        } catch (error) {
